...
func TestBufferRegistryStableAllocationAndRouting(t *testing.T) {
	...
	b1, _, _, err := ms.UpsertBufferRegistry(ctx, n1.ID, "#go", BufferChannel)
	if err != nil {
		t.Fatal(err)
	}
	b1again, _, _, err := ms.UpsertBufferRegistry(ctx, n1.ID, "#go", BufferChannel)
	if err != nil {
		t.Fatal(err)
	}
	b2, _, _, err := ms.UpsertBufferRegistry(ctx, n2.ID, "#go", BufferChannel)
	if err != nil {
		t.Fatal(err)
	}

	if b1 != b1again {
		t.Fatalf("buffer id changed: %d vs %d", b1, b1again)
	}
	if b1 == b2 {
	...
	if networkID != n1.ID || name != "#go" || kind != BufferChannel {
		t.Fatalf("unexpected routing: network=%d name=%q kind=%q", networkID, name, kind)
	}
}
...
func TestHistoryRoutingByGlobalBufferID(t *testing.T) {
	...
	global1, _, _, _ := ms.UpsertBufferRegistry(ctx, n1.ID, "#go", BufferChannel)
	global2, _, _, _ := ms.UpsertBufferRegistry(ctx, n2.ID, "#go", BufferChannel)
	log1, _ := ms.LogStore(n1.ID)
	log2, _ := ms.LogStore(n2.ID)
	local1, _, _, _ := UpsertLogBuffer(ctx, log1.DB, n1.ID, "#go", BufferChannel)
	local2, _, _, _ := UpsertLogBuffer(ctx, log2.DB, n2.ID, "#go", BufferChannel)
	_, _, _, _ = InsertLogMessage(ctx, log1.DB, LogMessageInput{BufferID: local1, Sender: "alice", Kind: "privmsg", Content: "hello libera"})
	_, _, _, _ = InsertLogMessage(ctx, log2.DB, LogMessageInput{BufferID: local2, Sender: "bob", Kind: "privmsg", Content: "hello oftc"})
