...
`buffer_settings` stores per-buffer display preferences keyed by buffer ID with `ON DELETE CASCADE`. Columns: `buffer_id` (PK, FK to `buffer_registry`), `show_embeds` (default 1), `show_presence_events` (default 1), `collapse_presence_events` (default 0), `pinned` (default 0), `updated_at`, `archived` (default 0). Channel and query buffers are eligible for settings; status buffers are rejected. `archived` drives the sidebar Archive section: the IRC runtime sets it on self-part/kick and clears it on self-join (`irc.syncBufferArchived`); a new message to an archived query clears it; disconnects never touch it.
