...
func TestErgoMessageSender(t *testing.T) {
	...
	limit := messageLimit(sender, channel)
	if limit < 4 || limit >= 400 {
		t.Fatalf("unexpected payload limit for bundled Ergo config: %d", limit)
	}
	t.Logf("negotiated payload limit: %d bytes", limit)
	handler := messageHandler(func() int { return messageLimit(sender, channel) }, func(message string) error {
		sender.Cmd.Message(channel, message)
		return nil
	})
	for _, content := range []string{strings.Repeat("x", limit+1), strings.Repeat("x", 400)} {
		response := httptest.NewRecorder()
		handler.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/message", strings.NewReader(content)))
		if response.Code != http.StatusBadRequest {
			t.Fatalf("oversized body (%d bytes): status = %d", len(content), response.Code)
		}
	}
	// A final short message acts as an ordering barrier: if any earlier request
	// is split or a rejected body is sent, the next receive sees the extra line.
	for _, content := range []string{
		"first message",
		strings.Repeat("x", limit),
		" " + strings.Repeat("x", limit-2) + " ",
		strings.Repeat("é", limit/2) + strings.Repeat("x", limit%2),
		"final message",
	} {
		response := httptest.NewRecorder()
		handler.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/message", strings.NewReader(content)))
